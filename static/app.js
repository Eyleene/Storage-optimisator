...
async function loadLocations(page = 1) {
  ...
  if (allLocations.length === 0) {
    ...
    if (!l || l.error) { el.innerHTML = "<i>Помилка завантаження</i>"; return []; }
    allLocations = l;
  }
...
async function createLocation(form) {
  ...
  if (r.error) alert(r.error);
  else { form.reset(); allLocations = []; loadLocations(); }
}
...
async function saveEditLocation(e) {
  ...
  if (r.error) alert(r.error);
  else { f.style.display = "none"; allLocations = []; loadLocations(); }
}
...
async function deleteLocation(id) {
  ...
  if (r.error) alert(r.error);
  else { allLocations = []; loadLocations(); }
}
...
document.getElementById("receive-form").addEventListener("submit", async e => {
  ...
  if (r.error) alert(r.error);
  f.reset(); await loadStock(); await loadTrans();
});
